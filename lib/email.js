...
export async function sendBudgetAlertEmail(userEmail, budgetData) {
  try {
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .header { background-color: ${status === "EXCEEDED" ? "#dc2626" : "#f59e0b"}; color: white; padding: 20px; border-radius: 8px 8px 0 0; }
            .content { background-color: #f9fafb; padding: 20px; border-radius: 0 0 8px 8px; }
            .budget-info { background-color: white; padding: 15px; border-radius: 4px; margin: 15px 0; }
            .stat { display: flex; justify-content: space-between; margin: 10px 0; }
            .stat-label { font-weight: 600; }
            .stat-value { font-weight: 700; color: ${status === "EXCEEDED" ? "#dc2626" : "#f59e0b"}; }
            .progress-bar { width: 100%; height: 20px; background-color: #e5e7eb; border-radius: 10px; overflow: hidden; margin: 15px 0; }
            .progress-fill { height: 100%; background-color: ${status === "EXCEEDED" ? "#dc2626" : "#f59e0b"}; width: ${Math.min(percentage, 100)}%; }
        ...
        <body>
          <div class="container">
            <div class="header">
              <h1>${status === "EXCEEDED" ? "🚨 Budget Exceeded" : "⚠️ Budget Warning"}</h1>
            </div>
            <div class="content">
              <p>Hello,</p>
              <p>Your budget for <strong>${categoryName || "Overall Budget"}</strong> has ${status === "EXCEEDED" ? "been exceeded" : "reached a warning threshold"}.</p>
              
              <div class="budget-info">
                <div class="stat">
                  <span class="stat-label">Budget Amount:</span>
                  <span class="stat-value">₹${budgetAmount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
                </div>
                <div class="stat">
                  <span class="stat-label">Amount Spent:</span>
                  <span class="stat-value">₹${spent.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
                </div>
                <div class="stat">
                ...
                <div class="stat">
                  ...
                  <span class="stat-value">₹${overspent.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
                </div>
                ` : `
                <div class="stat">
                  <span class="stat-label">Remaining:</span>
                  <span class="stat-value">₹${remaining.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
                </div>
                `}
    ...
    const result = await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL || "Finac Bank <budgets@finacbank.com>",
      to: userEmail,
      subject,
      html: htmlContent,
    });
