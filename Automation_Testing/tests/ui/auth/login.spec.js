...
test.describe('Login Tests', () => {
  ...
  test('should display login page elements', async () => {
    await expect(loginPage.page.locator(loginPage.selectors.cardTitle)).toBeVisible();
    ...
    await expect(loginPage.page.locator(loginPage.selectors.signInButton)).toBeVisible();
  });
  ...
  test('should show error for invalid credentials', async () => {
    ...
    const errorVisible = await loginPage.page.locator('//*[contains(@role, "status")] | //*[contains(@class, "error")]').isVisible();
  ...
  test('should validate invalid email format', async () => {
    const usersData = TestDataHelper.getUsersData();
    const invalidUser = usersData.invalidUsers.find(u => u.email === 'invalid-email');
